...
class Morph(BaseMorph):
    ...
    def level_up(self, num_levels: int, tableindex: int, home_pval: Union[str, Tuple[str, str]]):
        self.set_targetstats("characters/growth-rates", home_pval, ["Name"], tableindex)
    ...
    def promote(self, tableindex: int, home_pval: Union[str, Tuple[str, str]], promocls: str):
        self.set_targetstats("classes/promotion-gains", home_pval, ["Class"], tableindex)
        self.target_stats.pop(["Promotion"])
        self.target_stats.reindex(index=self.current_stats.index, fill_value=0.0)
        self.current_stats += self.target_stats * 1.0
        self.current_clstype = "classes/promotion-gains"

    def cap_stats(self, tableindex: int, home_pval: Union[str, Tuple[str, str]]):
